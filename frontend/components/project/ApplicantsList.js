      ...
      {applicants.map((applicant) => {
        ...
        return (
          <div
            ...
            className="rounded-2xl border border-border bg-card p-6 shadow-sm transition-all duration-300 hover:border-border-accent hover:shadow-[0_10px_30px_rgba(0,0,0,0.05)] hover:-translate-y-1"
          >
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex items-center gap-4">
                {/* Avatar */}
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-primary to-accent text-[15px] font-bold text-white shadow-sm">
                  {name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <h4 className="text-[17px] font-semibold text-foreground">{name}</h4>
              ...
              <div className="flex items-center gap-3">
                {/* PFI Score */}
                <div className="rounded-[10px] bg-surface px-4 py-2 text-center border border-border">
                  <div className="text-[12px] font-semibold uppercase tracking-wider text-muted">PFI</div>
                  <div className="text-[15px] font-bold text-primary">{pfiScore}</div>
                </div>

                {/* Success Rate */}
                {successRate != null && (
                  <div className="rounded-[10px] bg-surface px-4 py-2 text-center border border-border">
                    <div className="text-[12px] font-semibold uppercase tracking-wider text-muted">Success</div>
                    <div className="text-[15px] font-bold text-success">{successRate}%</div>
                  </div>
                )}

                {/* Accept button */}
                {applicant.accepted ? (
                  <span className="rounded-[10px] bg-success-subtle px-[18px] py-[10px] text-[15px] font-semibold text-success">
                    ✓ Accepted
                  </span>
                ) : (
                  <button
                    ...
                    className="rounded-[10px] bg-primary px-[18px] py-[10px] text-[15px] font-semibold text-white transition-all hover:bg-primary-hover hover:shadow-[0_4px_14px_rgba(79,110,247,0.3)] disabled:cursor-not-allowed disabled:opacity-60"
                  >
            ...
            {/* Application message */}
            {message && (
              <p className="mt-4 rounded-[10px] bg-surface px-5 py-3 text-[15px] font-medium text-muted italic border border-border">
                &ldquo;{message}&rdquo;
              </p>
            )}
          </div>
